...
describe('User api', () => {
  ...
  describe('Post: (/users) - Create a user', () => {
    ...
    it('Should fail if user wants to register as admin', (done) => {
      testUserParam.RoleId = String(adminRole.id);
      testUserParam.email = 'test@user.com';
      testUserParam.password = 'password';
      request.post('/users')
        .send(testUserParam)
        .expect(401)
        .end((err, res) => {
